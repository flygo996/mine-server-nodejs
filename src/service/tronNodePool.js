...
class MyTronWeb extends TronWeb {
  ...
  doPing() {
    ...
    return this.fullNode
      .request('wallet/getnowblock', {}, 'post')
      .then(() => {
        //测试solidityNode
        if (IGNORE_SOLIDITY_NODE_SCAN === false) {
          return this.solidityNode.request(
...
let getTronWebBetter = () => {
  ...
  if (
    betterNode != null &&
    betterNode.name !== 'master' &&
    masterNode != null &&
    masterNode.pingAvg - betterNode.pingAvg < 100
  ) {
...
let getTronWeb = name => {
  ...
  if (_.isEmpty(name) || name === 'default' || name === 'DEFAULT') {
    ...
    let _node = _.find(TRON_WEB_POOL, node => {
      return (
        node.name === name && node.state != null && node.state === TRONWEB_NODE_STATE.NODE_ENABLE
      )
    })
    return _node
  }
